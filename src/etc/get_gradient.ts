...
export default function getGradient() {
  ...
  const index = Math.floor((Math.random() * 1000) % colorLength);
  // console.log(`index=${index} colorLength=${colorLength}`);
  let { colors } = colorSet[index];
  let angle = Math.round(Math.random() * 360);

  let gradient = `linear-gradient(${angle}deg, ${colors[0]}40, ${colors[1]}40)`;
